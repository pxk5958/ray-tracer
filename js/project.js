...
vec3 castRay(vec3 rayOrigin, vec3 rayDir) {
    ...
    float kReflect = 1.0;  // initial reflection mix factor is 1.0
...
`
        ...
        for (int i = 0; i < numTriangles; i++) {
            ...
            if(intersectTriangle(rayOrigin, rayDir, hitInfo, vertices) && (hitInfo.t < tMin)) {
                hitInfo.hit = true;
        ...
        if (hitInfo.hit) {
            ...
            accumulatedColor += kReflect * bgColor;
            break;
        }
    
        float newKReflect = 0.0;
        accumulatedColor += kReflect * illuminate(hitInfo, rayDir, newKReflect);
        
        if (newKReflect <= EPSILON) {
            break;
        }
        // set up for next reflected ray
        kReflect *= newKReflect;
...
class Phong extends Material {
    constructor(id, ka, kd, ks, ke, kReflect, Co, Cs) {
...
class PhongCheckered extends Material {
    constructor(id, ka, kd, ks, ke, kReflect, Co1, Co2, Cs) {
...
function generateScene(meshes) {
    ...
    var sphere1Material = new Phong(nextPhongId++, 0.7, 0.6, 0.7, 16, 0,
        new THREE.Vector3(0.75, 0.1, 0.1), 
        new THREE.Vector3(0.7, 0.7, 0.7));
        
    var sphere2Material = new Phong(nextPhongId++, 0.55, 0.7, 0.75, 8, 0.5,
        new THREE.Vector3(0.6, 0.6, 0.6), 
        new THREE.Vector3(0.7, 0.7, 0.7));
        
    var cubeMaterial = new Phong(nextPhongId++, 0.5, 0.9, 0.2, 24, 0,
        new THREE.Vector3(0.8, 0.1, 0.7), 
        new THREE.Vector3(0.7, 0.7, 0.7));
    
    var cylMaterial = new Phong(nextPhongId++, 0.5, 0.3, 0.9, 20, 0,
        new THREE.Vector3(0.85, 0.7, 0.1), 
        new THREE.Vector3(0.7, 0.7, 0.7));
    
    var floorMaterial = new PhongCheckered(nextPhongCheckeredId++, 0.5, 0.9, 0.2, 12, 0,
        new THREE.Vector3(1, 0, 0), new THREE.Vector3(1, 1, 0), 
        new THREE.Vector3(0.7, 0.7, 0.7));
    
    var meshMaterial = new Phong(nextPhongId++, 0.7, 0.6, 0.7, 16, 0,
        new THREE.Vector3(0.75, 0.1, 0.1), 
        new THREE.Vector3(0.7, 0.7, 0.7));
